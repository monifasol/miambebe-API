  ...
  Recipe.deleteMany()
    ...
    .then( () => {
        ...
        User.findOne({ email: adminEmail })
            .then( (writer) => {
                // Here we seed the dabatase with recipes for this user. 
                  ...
                  Recipe.create(
                    {
                      ...
                      tags: recipe.tags,
                      user: (writer && writer._id) || 0
                    }
                  )
                  .then( (createdRecipe) => { console.log(`Created recipe with id ${createdRecipe._id}.`) })
                  .catch((err) => console.log(`An error occurred creating a recipe to the DB: ${err}.`) )
                })
                console.log(`Created ${recipes.length} recipes.`)
            })
            .catch( (err) => {
              console.log(`No writer found! Error: ${err}`)
            })
    })
    .catch((err) =>
